...
function checkAnswers(auto = false) {
    ...
    if (score >= 3) {
        fetch('save_quiz_progress.php', {
            ...
            body: JSON.stringify({ score: score, exercise_number: 2, quiz_number: 6 })
        })
...
window.onload = function() {
    fetch('questions_bank.json')
        .then(res => res.json())
        .then(data => {
            // Filter for EPT2 Adjectives: quizID starts with 'B', category is 'adjectives'
            const filtered = data.questions.filter(q => q.quizID.startsWith('B') && q.category === 'adjectives');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add timer box if not present
    if (!document.getElementById('timerBox')) {
        const timerBox = document.createElement('div');
        timerBox.id = 'timerBox';
        timerBox.style.display = 'none';
        timerBox.innerHTML = '<span id="timer" style="font-size:1.5rem;font-weight:bold;"></span>';
