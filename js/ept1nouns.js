...
function checkAnswers(auto = false) {
    ...
    if (score >= 3) {
        fetch('save_quiz_progress.php', {
            ...
            body: JSON.stringify({ score: score, exercise_number: 1, quiz_number: 5 })
        })
...
window.onload = function() {
    fetch('questions_bank.json')
        .then(res => res.json())
        .then(data => {
            // Filter for EPT1 Nouns: quizID starts with 'A', category is 'nouns'
            let filtered = data.questions.filter(q => q.quizID.startsWith('A') && q.category === 'nouns');
            // Fallback: If no noun questions, use verbs as a placeholder
            if (filtered.length === 0) {
                filtered = data.questions.filter(q => q.quizID.startsWith('A') && q.category === 'verbs');
            }
            ...
            if (questions.length > 0) {
                ...
                document.getElementById("questionArea").innerHTML = '<div class="alert alert-warning">No noun questions available. Please add noun questions to questions_bank.json.</div>';
