...
function checkAnswers(auto = false) {
    ...
    if (score >= 3) {
        fetch('save_quiz_progress.php', {
            ...
            body: JSON.stringify({ score: score, exercise_number: 2, quiz_number: 3 })
        })
...
window.onload = function() {
    fetch('questions_bank.json')
        .then(res => res.json())
        .then(data => {
            // Filter for EPT2 Greetings: quizID starts with 'B', category is 'greetings'
            const filtered = data.questions.filter(q => q.quizID.startsWith('B') && q.category === 'greetings');
