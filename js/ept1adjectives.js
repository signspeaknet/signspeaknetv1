...
function checkAnswers(auto = false) {
    ...
    if (score >= 3) {
        fetch('save_quiz_progress.php', {
            ...
            body: JSON.stringify({ score: score, exercise_number: 1, quiz_number: 6 })
        })
...
window.onload = function() {
    fetch('questions_bank.json')
        .then(res => res.json())
        .then(data => {
            // Filter for EPT1 Adjectives: quizID starts with 'A', category is 'adjectives'
            const filtered = data.questions.filter(q => q.quizID.startsWith('A') && q.category === 'adjectives');
