...
function checkAnswers(auto = false) {
    ...
    if (score >= 3) {
        fetch('save_quiz_progress.php', {
            ...
            body: JSON.stringify({ score: score, exercise_number: 2, quiz_number: 7 })
        })
...
window.onload = function() {
    fetch('questions_bank.json')
        .then(res => res.json())
        .then(data => {
            // Filter for EPT2 Questions: quizID starts with 'B', category is 'questions'
            const filtered = data.questions.filter(q => q.quizID.startsWith('B') && q.category === 'questions');
