...
// Remove hardcoded questions and allChoices
// Add dynamic loader for questions_bank.json
let questions = [];
let allChoices = [];
...
function generateChoicesForQuestion(question) {
    // Use the options from the question_bank
    ...
    container.innerHTML = `
        ...
        ${labeledChoices.map(choice => `
          <div class="form-check">
            <input class="form-check-input" type="radio" name="question${index}" id="q${index}_${choice.label}" value="${choice.label}"
...
function checkAnswers(auto = false) {
    ...
    if (score >= 3) {
        fetch('save_quiz_progress.php', {
            ...
            body: JSON.stringify({
                score: score,
                exercise_number: 1, // Part 1
                quiz_number: 1 // Numbers quiz
            })
        })
...
window.onload = function() {
    fetch('questions_bank.json')
        .then(res => res.json())
        .then(data => {
            // Filter for EPT1 Numbers: quizID starts with 'A', category is 'numbers'
            const filtered = data.questions.filter(q => q.quizID.startsWith('A') && q.category === 'numbers');
